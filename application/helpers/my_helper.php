...
function settings() 
{
	...
	$setting= $CI->SettingModel->get_setting();
	$result = []; 
...
{	
	...
	foreach($_FILES as $name => $fileInfo)
	{
		...
		$config['upload_url'] =  base_url().'assets/image/Admin/';
		$config['allowed_types'] = "gif|jpg|jpeg|png|iso|dmg|zip|rar|doc|docx|xls|xlsx|ppt|pptx|csv|ods|odt|odp|pdf|rtf|sxc|sxi|txt|exe|avi|mpeg|mp3|mp4|3gp";
		$config['max_size'] = '2000000'; 
		$config['file_name'] = $newname;
		$CI->load->library('upload', $config);
		move_uploaded_file($tmpname,"assets/image/Admin/".$newname);
		return $newname;
	}
}
...
function is_login()
{
	...
	if(isset($_SESSION['user_details'])){
		$permission = isset($CI->session->get_userdata()['user_details'][0]->user_type)?$CI->session->get_userdata()['user_details'][0]->user_type:'';
		if ($permission == 'student' || $permission == 'Student') {
...
function news_url($url_title)
{
	return site_url('news/' . $url_title);
}

function events_url($url_title)
{
	return site_url('events/' . $url_title);
}
